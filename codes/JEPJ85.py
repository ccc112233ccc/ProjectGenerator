...
def JEPJ85(r_cond, t_mica, t_ins, t_wrap, t_sheath_in, t_armor, t_sheath_out, length, pos,
                          sigma_cond, sigma_shield, mu, epsr, tan_delta, freq):
    Nf = len(freq)
    N = pos.shape[0]
    Z0 = 50
    c0 = c
    rsh = (r_cond+t_mica+t_ins) * (2/np.sqrt(3) + 1)+t_wrap+t_sheath_in;
    Rsh = rsh+t_armor
...
def main():
    ...
    s_params, S_mixed, freq_out = JEPJ85(
        r_cond=6.5e-3,
        t_mica=1e-3,
        t_ins=5e-3,
        t_wrap=2e-3,
